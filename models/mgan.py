    ...
    def forward(self, batch_size, ctx, asp):
        ...
        for i, ctx_chunk in enumerate(ctx_chunks):
            for j, asp_chunk in enumerate(asp_chunks):
                ...
                asp_chunk = torch.as_tensor(asp_chunk,dtype=torch.float).to(self.opt.device)
                ctx_chunk = torch.as_tensor(ctx_chunk,dtype=torch.float).to(self.opt.device)
...
class MGAN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        alignment_mat = torch.as_tensor(alignment_mat,dtype=torch.float).to(self.opt.device)
        asp_out = torch.as_tensor(asp_out,dtype=torch.float).to(self.opt.device)
        ctx_out = torch.as_tensor(ctx_out,dtype=torch.float).to(self.opt.device)
        ctx_pool = torch.as_tensor(ctx_pool,dtype=torch.float).to(self.opt.device)
