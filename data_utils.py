    ...
    def __init__(self, fname, tokenizer):
        ...
        if "_cl_2X3" in fname:
            ...
            for i in range(0, len(lines), 3):
                ...
                if "mask" in fname:
                    text_left, _, text_right = [s.lower().strip() for s in lines[i].partition("[MASK]")]
                    tmp = "[MASK]"
                    aspect = lines[i + 1].lower().strip()
                    polarity = lines[i + 2].strip()

                    text_indices = tokenizer.text_to_sequence(text_left + " " + tmp + " " + text_right)
                    context_indices = tokenizer.text_to_sequence(text_left + " " + text_right)
                    left_indices = tokenizer.text_to_sequence(text_left)
                    left_with_aspect_indices = tokenizer.text_to_sequence(text_left + " " + tmp)
                    right_indices = tokenizer.text_to_sequence(text_right, reverse=True)
                    right_with_aspect_indices = tokenizer.text_to_sequence(tmp + " " + text_right, reverse=True)
                    aspect_indices = tokenizer.text_to_sequence(tmp)
                    left_len = np.sum(left_indices != 0)
                    aspect_len = np.sum(aspect_indices != 0)
                    aspect_boundary = np.asarray([left_len, left_len + aspect_len - 1], dtype=np.int64)
                    polarity = int(polarity) + 1
                    ...
                    text_len = np.sum(text_indices != 0)
                    concat_bert_indices = tokenizer.text_to_sequence(
                        '[CLS] ' + text_left + " " + tmp + " " + text_right + ' [SEP] ' + tmp + " [SEP]")
                    concat_segments_indices = [0] * (text_len + 2) + [1] * (aspect_len + 1)
                    concat_segments_indices = pad_and_truncate(concat_segments_indices, tokenizer.max_seq_len)

                    text_bert_indices = tokenizer.text_to_sequence(
                        "[CLS] " + text_left + " " + tmp + " " + text_right + " [SEP]")
                    aspect_bert_indices = tokenizer.text_to_sequence("[CLS] " + tmp + " [SEP]")
